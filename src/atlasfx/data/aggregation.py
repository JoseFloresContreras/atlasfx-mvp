...
def aggregate_data(
    data: pd.DataFrame, time_window: str, aggregators: dict[str, Callable]
) -> pd.DataFrame:
    """
...
def process_single_file(
    input_file: str, time_window: str, aggregators: dict[str, Callable]
) -> tuple:
    """
    ...
    try:
        ...
        log.info(
            f"Aggregated data memory usage: {aggregated_data.memory_usage(deep=True).sum() / 1024**2:.2f} MB"
        )
...
def combine_aggregated_dataframes(
    dataframes_dict: dict[str, pd.DataFrame], time_window: str
) -> pd.DataFrame:
    """
    ...
    log.info(
        f"Combined dataframe memory usage: {combined_df.memory_usage(deep=True).sum() / 1024**2:.2f} MB"
    )
...
def run_aggregate(config):
    ...
    try:
        ...
        for input_file in input_files:
            success, symbol, aggregated_data = process_single_file(
                input_file, time_window, aggregators
            )
        ...
        if dataframes_dict:
            ...
            if not combined_df.empty:
                ...
                log.info(
                    f"✅ Successfully saved combined data with {len(combined_df)} rows to {output_path}"
                )
            else:
